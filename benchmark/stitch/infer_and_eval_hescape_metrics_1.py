...
def _hash_whitelist(id_whitelist: set[str]) -> str:
    """
    Stable 40-hex SHA1 of the sorted whitelist IDs.
    Truncated to 10 chars for directory naming.
    """
    s = "\n".join(sorted(map(str, id_whitelist)))
    return hashlib.sha1(s.encode("utf-8")).hexdigest()[:10]

def _cache_root_from_cfg(cfg: dict) -> str:
    """
    Where to store cached filtered datasets.
    Priority: cfg['cache_root'] → cfg['out_dir']/_cache → ./_cache
    """
    if cfg.get("cache_root"):
        return str(cfg["cache_root"])
    if cfg.get("out_dir"):
        return str(Path(cfg["out_dir"]) / "_cache")
    return str(Path.cwd() / "_cache")

def _whitelist_cache_dir(cfg: dict, id_whitelist: set[str], id_field: str | None) -> str:
    """
    Build a unique cache dir using panel/split/id_field and whitelist hash.
    """
    root = Path(_cache_root_from_cfg(cfg))
    parts = [
        str(cfg["panel"]),
        str(cfg["split"]),
        str(id_field or "auto"),
        _hash_whitelist(id_whitelist),
    ]
    # make it readable but unique
    name = "__".join(parts)
    return str(root / name)


def materialize_whitelist_dataset(panel: str,
                                  split: str,
                                  id_field: str,
                                  id_whitelist: set[str],
                                  keep_columns: list[str] | None = None,
                                  save_dir: str | None = None,
                                  log_every: int = 2000):
    """
    Stream-filter the HF split to keep only whitelist sample-IDs,
    then build a non-streaming `datasets.Dataset` from those rows.

    If `save_dir` is given, saves the dataset there for reuse.
    """
    assert id_whitelist and len(id_whitelist) > 0, "id_whitelist must be non-empty"

    # 1) open streaming iterator
    ds_stream = load_dataset("Peng-AI/hescape-pyarrow", name=panel, split=split, streaming=True)
    features = ds_stream.features

    # if caller didn’t pass an id_field, try to detect
    if not id_field:
        for c in ("id", "dataset_id", "name"):
            if c in features:
                ...
                break
        if not id_field:
            raise ValueError("Could not detect id_field; set cfg['id_field'].")

    # we’ll accumulate columns into lists
    buf = {}
    n_kept = 0

    remaining = set(id_whitelist)
    STOP_AFTER_MISSES = 1000
    miss_streak = 0

    def _append_row(row):
        nonlocal buf, n_kept
        # select columns to keep (or all)
        cols = keep_columns if keep_columns else list(row.keys())
        for c in cols:
            v = row[c]
            # Convert arrays to numpy (HF handles nested lists fine, but be consistent)
            buf.setdefault(c, []).append(v)
        n_kept += 1
        if (n_kept % log_every) == 0:
            print(f"[materialize] kept {n_kept} rows…", flush=True)

    # 2) filter in streaming
    for r in ds_stream:
        sid = _get_sample_id_from_row(r, id_field, features)

        if sid in id_whitelist:
            _append_row(r)
            if sid in remaining:
                remaining.remove(sid)
                miss_streak = 0
        else:
            if not remaining:  # all whitelist IDs already seen
                miss_streak += 1
                if miss_streak == 1:
                    print("[materialize] all whitelist IDs seen; starting miss-streak...", flush=True)
                if miss_streak >= STOP_AFTER_MISSES:
                    print(f"[materialize] early-stop after {STOP_AFTER_MISSES} consecutive non-whitelist rows.", flush=True)
                    break


    if n_kept == 0:
        raise RuntimeError("[materialize] No rows matched id_whitelist. Check id_field / whitelist.")

    # 3) build Dataset (timing)
    import time
    t0 = time.time()
    ds = Dataset.from_dict(buf)
    print(f"[materialize] from_dict took {time.time() - t0:.1f}s", flush=True)

    # 4) optional save for reuse (timing)
    if save_dir:
        t1 = time.time()
        os.makedirs(save_dir, exist_ok=True)
        ds.save_to_disk(save_dir)
        print(f"[materialize] save_to_disk took {time.time() - t1:.1f}s → {save_dir}", flush=True)
...
def run_from_cfg(user_cfg: dict):
    ...
    if bool(cfg["streaming"]):
        ...
        '''
        #new for test
        # --- NON-STREAMING PATH WITH WHITELIST CACHING/REUSE ---
        print("[stage] entering non-streaming mode…", flush=True)

        # Temporary stream to detect fields
        tmp_stream = load_dataset(
            "Peng-AI/hescape-pyarrow",
            name=cfg["panel"],
            split=cfg["split"],
            streaming=True,
        )
        features = tmp_stream.features
        id_field = _detect_id_field(features, cfg.get("id_field"))

        ds = None
        
        if id_whitelist:
            save_dir = _whitelist_cache_dir(cfg, id_whitelist, id_field)
        else:
            save_dir = None
        


        # ✅ 1. Try to reuse cached dataset
        if save_dir and os.path.exists(save_dir):
            print(f"[reuse] found cached whitelist dataset at {save_dir}", flush=True)
            ds = load_from_disk(save_dir)
            print(f"[reuse] loaded cached dataset with {len(ds)} rows", flush=True)

        # ⚙️ 2. Otherwise, materialize it
        if ds is None:
            if id_whitelist:
                print("[materialize] building a local non-streaming dataset from whitelist via streaming…", flush=True)
                keep_cols = None  # or subset of columns to save space
                ds = materialize_whitelist_dataset(
                    panel=cfg["panel"],
                    split=cfg["split"],
                    id_field=id_field,
                ...
                print(f"[materialize] final local dataset size: {len(ds)} rows", flush=True)
            else:
        ...
        if len(ds) == 0:
            raise RuntimeError("Dataset vuoto dopo filtro/materializzazione.")

        # same as before ↓
        tds = HescapeTorchDataset(ds)

        # === HVG with caching ===
        hvg_cache = Path(save_dir).joinpath(f"hvg_{int(cfg['hvg_k'])}.npy") if save_dir else None

        if hvg_cache and hvg_cache.exists():
            print(f"[hvg] loading cached HVG indices from {hvg_cache}", flush=True)
            hvg_idx = np.load(hvg_cache)
            assert len(hvg_idx) == int(cfg["hvg_k"])
        else:
            print(f"[hvg] fitting HVG on {len(ds)} filtered samples...", flush=True)
            G_all = np.stack([np.array(r["gexp"], dtype=np.float32).squeeze() for r in ds], axis=0)
            hvg_idx = select_hvg_indices(G_all, int(cfg["hvg_k"]))
            if hvg_cache:
                hvg_cache.parent.mkdir(parents=True, exist_ok=True)
                np.save(hvg_cache, hvg_idx)
                print(f"[hvg] cached HVG indices to {hvg_cache}", flush=True)
    ...
    if bool(cfg.get("use_inception", False)) and int(cfg.get("image_pca_dim", 0)) > 0:
        ...
        print(f"[pca] warm-up target: {pca_fit_n} image features...", flush=True)

        feats_buf = []
        bs = int(cfg["batch_size"])

        # --- CASE A: Non-streaming path (use local cached/whitelisted dataset `ds` / `tds`) ---
        if ds_eval_stream is None:
            print("[pca] using local cached dataset for PCA warm-up", flush=True)

            # Optionally cache the PCA model itself on disk (keyed by whitelist hash, panel, split, dim)
            save_dir = _whitelist_cache_dir(cfg, id_whitelist, _detect_id_field(features, cfg.get("id_field"))) if id_whitelist else None
            pca_cache = Path(save_dir).joinpath(f"pca_{cfg['image_pca_dim']}.joblib") if save_dir else None
            ...
            if joblib and pca_cache and pca_cache.exists():
                print(f"[pca] loading cached PCA from {pca_cache}", flush=True)
                from sklearn.decomposition import PCA  # ensure class available
                pca_model = joblib.load(pca_cache)
            else:
                # Small loader just to extract image feats quickly
                pca_loader = DataLoader(
                    tds, batch_size=bs, shuffle=True,
                    num_workers=int(cfg["num_workers"]), pin_memory=bool(cfg["pin_memory"])
                )
                seen = 0
                with torch.no_grad():
                    for batch in pca_loader:
                        # `batch` is a list of HF-like rows in your HescapeTorchDataset
                        imgs = torch.stack([img_tf(b["image"] if hasattr(b["image"], "size")
                                                else Image.fromarray(np.array(b["image"])).convert("RGB"))
                ...
                if not feats_buf:
                    raise RuntimeError("[pca] no features collected from local dataset.")
                X_fit = np.concatenate(feats_buf, axis=0)[:pca_fit_n]
                ...
                if joblib and pca_cache:
                    pca_cache.parent.mkdir(parents=True, exist_ok=True)
                    joblib.dump(pca_model, pca_cache)
                    print(f"[pca] cached PCA to {pca_cache}", flush=True)

        # --- CASE B: Streaming path (your current logic with whitelist filter) ---
        else:
            print("[pca] using streaming iterator for PCA warm-up", flush=True)
            ...
            pca_features = pca_stream.features
            pca_id_field = _detect_id_field(pca_features, cfg.get("id_field"))

            remaining_pca = set(id_whitelist) if id_whitelist else None
            miss_streak_pca = 0
            STOP_AFTER_MISSES = 1000
            ...
            with torch.no_grad():
                for rec in pca_stream:
                    if id_whitelist:
                        sid = _get_sample_id_from_row(rec, pca_id_field, pca_features)
                        if sid not in id_whitelist:
                            if remaining_pca is not None and len(remaining_pca) == 0:
                                miss_streak_pca += 1
                                if miss_streak_pca >= STOP_AFTER_MISSES:
                                    print(f"[pca] early-stop after {STOP_AFTER_MISSES} consecutive non-whitelist rows.", flush=True)
                                    break
                            continue
                        if remaining_pca is not None and sid in remaining_pca:
                            remaining_pca.remove(sid)
                            miss_streak_pca = 0
    ...
    if cfg.get("out_dir"):
        ...
        print("[export] out_dir not set → skipping metrics/embeddings export to disk.")
