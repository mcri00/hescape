...
class ImageEncoder(nn.Module):
    ...
    def forward(self, image_feature):
        ...
        return image_hidden
'''    
# Adapted for dynamic input image features size
class ImageEncoder(nn.Module):
    def __init__(self, in_dims, hidden_dims, intermediate_dim=512):
        super().__init__()
        self.in_features = in_dims
        self.hidden_features = hidden_dims

        self.encoder = nn.Sequential(
            nn.Linear(in_dims, intermediate_dim),
            nn.BatchNorm1d(intermediate_dim),
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.Linear(intermediate_dim, hidden_dims),
            nn.BatchNorm1d(hidden_dims),
            nn.ReLU(),
        )

    def forward(self, image_feature):
        return self.encoder(image_feature)
...
class MuCST(nn.Module):
    def __init__(self, gene_dims, img_dims, graph_nei, intermediate_dim, project_dims=[64, 64]):
        super().__init__()
        ...
        self.img_encoder = ImageEncoder(in_dims=img_dims[0], hidden_dims=img_dims[-1], intermediate_dim=intermediate_dim)
...
def load_mucst_from_cfg(cfg: dict, input_dims_img: int, input_dims_gene: int, adj_norm_np: np.ndarray, device="cuda"):
    """
    Istanzia MuCST con dimensioni coerenti al training e carica checkpoint.
    """
    proj_dims       = list(map(int, cfg.get("mucst_proj_dims", [64, 64])))
    intermediate_dim= int(cfg.get("intermediate_dim", 512))
    hidden_dim      = int(cfg.get("hidden_dim", proj_dims[0]))  # nel tuo training spesso hidden = proj[0]

    # gene_dims: [in, hidden]   img_dims: [in, hidden]
    gene_dims = [int(input_dims_gene), int(hidden_dim)]
    img_dims  = [int(input_dims_img),  int(hidden_dim)]

    adj_t = torch.tensor(adj_norm_np, dtype=torch.float32, device=device)

    model = MuCST(
        gene_dims=gene_dims,
        img_dims=img_dims,
        graph_nei=adj_t,                 # nel tuo costruttore la tieni come self.graph
        intermediate_dim=intermediate_dim,
        project_dims=proj_dims
    ).to(device).eval()
    ...
    # accetta sia {'state_dict': ...} sia lo state_dict diretto
    state_dict = sd.get("state_dict", sd)
    # Se addestrato in DataParallel, rimuovi 'module.'
    new_state = {}
