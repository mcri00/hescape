...
def run_from_cfg(user_cfg: dict):
    ...
    if int(cfg.get("image_pca_dim", 0)) > 0:
        ...
        print(f"[pca] Inception features {raw_img_feat_dim} → PCA {want}")

    # === build mclSTExp model (ATTENTION variant) ===
    assert "checkpoint" in cfg, "cfg must contain 'checkpoint'"
    muse_cfg = {
        "checkpoint": cfg["checkpoint"],
        "input_dim_image": int(cfg["input_dim_image"]),                # must match extractor/PCA
        "input_dim_transcriptome": int(cfg["input_dim_transcriptome"]),# == hvg_k
        "n_hidden_muse": int(cfg.get("n_hidden_muse", 128)),
        "dim_z_muse": int(cfg.get("dim_z_muse", 256)),                 # dim_z
    }
    model = load_muse_from_cfg(muse_cfg).to(device).eval()
    print("[model] MUSE ready.", flush=True)
        ...
        if bool(cfg["streaming"]):
            ...
            for rec in ds_eval:
                ...
                if len(buffer) == bs:
                    ...
                    _x, _y, _z, encode_x, encode_y = model(g_tensor.to(device).float(),
                                           img_feats.to(device).float())
